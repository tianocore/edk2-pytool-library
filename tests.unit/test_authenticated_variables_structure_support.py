...
def sha256_esl_builder(initList: []):
    ...
    for entry in initList:
        ...
        esl.AddSignatureData(EfiSignatureDataEfiCertSha256(digest=hashBytes, sigowner=uuid.UUID(ownerGuidStr)))
    return esl
...
def x509_esl_builder(initList: []):
    ...
    if listSize == 0:
        ...
        esl.AddSignatureData(EfiSignatureDataEfiCertX509(cert=certBytes, sigowner=uuid.UUID(ownerGuidStr)))
    return esl
...
def MixedCertsData():
    ...
    esls = [x509_esl_builder([(DEBIAN_CERT, owner)]), x509_esl_builder([(UBUNTU_CERT, owner)])]

    input = [esls[1], esls[0], esls[1], esls[0], esls[1]]
    esd = EfiSignatureDatabase(esl_list=input)

    dupes = [esls[0], esls[1], esls[1]]
