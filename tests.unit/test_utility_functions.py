...
class UtilityFunctionsTest(unittest.TestCase):
    def test_RunPythonScript(self):  # simple- run yourself!
        ...
        ret = utilities.RunPythonScript(
            path, "", capture=False, workingdir=working_dir, raise_exception_on_nonzero=True
        )
        self.assertEqual(ret, 0)
        # now try a bad path
        ...
        with self.assertRaises(Exception):
            ret = utilities.RunPythonScript(
                bad_path, "", capture=False, workingdir=None, raise_exception_on_nonzero=True
            )
            self.assertNotEqual(ret, 0)
...
class TestRemoveTree:
    ...
    def test_long_path_remove_tree(self, tmp_path):
        ...
        if value == 1:
            pytest.skip(
                r"Long paths are enabled. To run the test, disable long paths with the registry key located at: HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystemLongPathsEnabled"
            )  # noqa: E501
