...
class Rule(object):
    ...
    @classmethod
    def FromFsAndVtOffset(cls: "Rule", fs: BinaryIO, vtOffset: int) -> "Rule":
        ...
        (RootKey, OffsetToSubKeyName, OffsetToValueName, OffsetToValue) = struct.unpack(
        ...
        return cls(
            RootKey=RootKey,
            OffsetToSubKeyName=OffsetToSubKeyName,
            OffsetToValueName=OffsetToValueName,
    ...
    @classmethod
    def FromFsAndVtBytes(cls: "Rule", fs: BinaryIO, vt: bytes) -> "Rule":
    ...
    def Serialize(self, ruleOut: bytearray, valueOut: bytearray, offsetInVT: int) -> None:
        """Serialize Rule.
        ...
        ruleOut += struct.pack(
            self.StructFormat, self.RootKey, self.OffsetToSubKeyName, self.OffsetToValueName, self.OffsetToValue
        )
...
class PolicyValueType:
    ...
    SupportedValueTypes = {POLICY_VALUE_TYPE_DWORD, POLICY_VALUE_TYPE_QWORD, POLICY_VALUE_TYPE_STRING}
    ...
    @classmethod
    def FromFileStream(cls: "PolicyValueType", fs: BinaryIO, fsOffset: int = None) -> "PolicyValueType":
    ...
    @classmethod
    def FromBytes(cls: "PolicyValueType", b: bytes, bOffset: int = 0) -> "PolicyValueType":
    ...
    def Serialize(self, valueOut: bytearray) -> None:
    ...
    def Get(self: "PolicyValueType") -> int:
...
class PolicyString:
    ...
    @classmethod
    def FromFileStream(cls: "PolicyString", fs: BinaryIO, fsOffset: int = None) -> "PolicyString":
        ...
        LocalString = bytes.decode(fs.read(StringLength), encoding="utf_16_le")
        if len(LocalString) != (StringLength / 2):
            raise Exception("String length mismatch")
        return cls(String=LocalString)

    @classmethod
    def FromBytes(cls: "PolicyString", b: bytes, bOffset: int = 0) -> "PolicyString":
        ...
        LocalString = bytes.decode(b[bOffset : bOffset + StringLength], encoding="utf_16_le")
        if len(LocalString) != (StringLength / 2):
            raise Exception("String length mismatch")
        return cls(String=LocalString)
...
class PolicyValue:
    ...
    @classmethod
    def FromFileStream(cls: "PolicyValue", fs: BinaryIO, fsOffset: int = None) -> "PolicyValue":
    ...
    @classmethod
    def FromBytes(cls: "PolicyValue", b: bytes, bOffset: int = 0) -> "PolicyValue":
...
class Reserved2(object):
    ...
    def PopulateFromFileStream(self, fs: BinaryIO, vtOffset: int = 0) -> None:
        ...
        (self.ObjectType, self.Element, self.OffsetToValue) = struct.unpack(self.StructFormat, fs.read(self.StructSize))
    ...
    def Serialize(self, ruleOut: bytearray, valueOut: bytearray = None, offsetInVT: int = 0) -> None:
...
class FirmwarePolicy(object):
    ...
    FW_POLICY_VALUE_STATE_STRINGS = {FW_POLICY_VALUE_STATE_TBD: "To Be Defined Placeholder"}
    ...
    def SetDevicePolicy(self, policy: int) -> None:
        ...
        rule = Rule(RootKey=self.FW_POLICY_ROOT_KEY, SubKeyName=SubKeyName, ValueName=ValueName, Value=Value)
        self.AddRule(rule)

    def SetDeviceTarget(self, target: dict) -> None:
        ...
        for k, v in target.items():
            ...
            rule = Rule(
                RootKey=self.FW_POLICY_ROOT_KEY,
    ...
    def SerializeToStream(self, stream: BinaryIO) -> None:
    ...
    def Serialize(self, output: bytearray) -> None:
        ...
        fixedSizeHeader = struct.pack(
            self.FixedStructFormat,
            self.FormatVersion,
            self.PolicyVersion,
            self.PolicyPublisher.bytes_le,
            self.Reserved1Count,
            self.OptionFlags,
            self.Reserved2Count,
            self.RulesCount,
    ...
    def FromFileStream(self, fs: BinaryIO, parseByBytes: bool = True) -> None:
        ...
        if self.FormatVersion > self.POLICY_FORMAT_VERSION:
            print("Policy Format Version %x is not supported" % self.FormatVersion)
            raise Exception("Policy Format Version is newer than supported")

        self.PolicyVersion = struct.unpack("<I", fs.read(4))[0]
        PolicyPublisher = struct.unpack("<16s", fs.read(struct.calcsize("<16s")))[0]
        ...
        for i in range(self.Reserved1Count):
            Reserved1 = struct.unpack("<16s", fs.read(struct.calcsize("<16s")))[0]
        ...
        self.Reserved2 = []
        for i in range(self.Reserved2Count):
            self.Reserved2.append(Reserved2(fs=fs, vtOffset=self.ValueTableOffset))
    ...
    def PrintDevicePolicy(self, devicePolicy: int, prefix: str = "") -> None:
    ...
    def Print(self) -> None:
        ...
        for rule in self.Rules:
            rule.Print(prefix=prefix)
            if (
                rule.RootKey == self.FW_POLICY_ROOT_KEY
